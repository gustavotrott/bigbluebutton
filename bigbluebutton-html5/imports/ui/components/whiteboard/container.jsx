...
const WhiteboardContainer = (props) => {
  ...
  useSubscription(ANNOTATION_HISTORY_STREAM, {
    ...
    onData: ({ data: subscriptionData }) => {
      ...
      shapesToProcessQueueRef.current.push(validShapes.length > 0 ? validShapes : []);
    },
  });
  ...
  const processAnnotations = (data) => {
    ...
    data.forEach((item) => {
      if (!item.annotationInfo) {
        annotationsToRemove.push(item.annotationId);
      } else {
        ...
        if (existingShape) {
          ...
          annotationsToRemove = annotationsToRemove.filter(
    ...
    shapesToRemoveQueueRef.current = annotationsToRemove.length > 0 ? annotationsToRemove : [];
  };
  ...
  useEffect(() => {
    if (!curPageId || initialPageAnnotationsLoading) {
      setInitialShapes([]);
      shapesToProcessQueueRef.current.push([]);
      shapesToRemoveQueueRef.current.push([]);
    }
